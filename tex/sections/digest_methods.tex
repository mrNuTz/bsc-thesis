...
\subsection{ssdeep}
ssdeep is a Context Triggered Piecewise Hash (CTPH) based on spamsum\cite{spamsum} wich was written for email spam detection. It is described in detail in the accompanying paper \aycite{kornblum2006identifying}
...
\subsection{jumpHash}
Inspired by ssdeep and the learnings from the solc-version-testet \cite{solc-versions-testset} I implemented jumpHash \cite{ethereum-contract-similarity}.
\begin{ol}
...
\end{lstlisting}
The Unicode character \code{ord("°")=0xb0} was chosen as \code{0} chunk hash because it is followed by 255 valid characters.
